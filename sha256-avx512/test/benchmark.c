...
static void wots_gen_pkx16(unsigned char *pk, const unsigned char *seed,
...
int main()
{
    ...
    unsigned char wots_pk[16*SPX_WOTS_PK_BYTES];
    ...
    MEASURE("  - WOTS pk gen x16.. ", (1 << SPX_TREE_HEIGHT) / 16, wots_gen_pkx16(wots_pk, sk, pk, (uint32_t *) addr));
    ...
    MEASURE("  - WOTS pk gen x16.. ", SPX_D * (1 << SPX_TREE_HEIGHT) / 16, wots_gen_pkx16(wots_pk, sk, pk, (uint32_t *) addr));
    MEASURE("Verifying..          ", 1, crypto_sign_open(mout, &mlen, sm, smlen, pk));
    }
    else
    {
    MEASURE("Generating keypair.. ", 1, crypto_sign_keypair(pk, sk));
    MEASURE("  - WOTS pk gen 8x.. ", (1 << SPX_TREE_HEIGHT) / 8, wots_gen_pkx16(wots_pk, sk, pk, (uint32_t *) addr));
    MEASURE("Signing..            ", 1, crypto_sign(sm, &smlen, m, SPX_MLEN, sk));
    MEASURE("  - FORS signing..   ", 1, fors_sign(fors_sig, fors_pk, fors_m, sk, pk, (uint32_t *) addr));
    MEASURE("  - WOTS pk gen x8.. ", SPX_D * (1 << SPX_TREE_HEIGHT) / 8, wots_gen_pkx16(wots_pk, sk, pk, (uint32_t *) addr));
    MEASURE("Verifying..          ", 1, crypto_sign_open(mout, &mlen, sm, smlen, pk));
    }
...
static void wots_gen_pkx16(unsigned char *pk, const unsigned char *seed,
                 const unsigned char *pub_seed, uint32_t addr[8]) {
    struct leaf_info_x16 leaf;
    unsigned steps[ SPX_WOTS_LEN ] = { 0 };
    INITIALIZE_LEAF_INFO_X16(leaf, addr, steps);
    wots_gen_leafx16(pk, seed, pub_seed, 0, &leaf);
}
