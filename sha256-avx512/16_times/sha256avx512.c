...
// Transpose 16 vectors containing 32-bit values
void transpose(u512 s[8]) {
    u512 tmp0[8];
    u512 tmp1[8];
    tmp0[0] = _mm512_unpacklo_epi32(s[0], s[1]);
    tmp0[1] = _mm512_unpackhi_epi32(s[0], s[1]);
    tmp0[2] = _mm512_unpacklo_epi32(s[2], s[3]);
    tmp0[3] = _mm512_unpackhi_epi32(s[2], s[3]);
    tmp0[4] = _mm512_unpacklo_epi32(s[4], s[5]);
    tmp0[5] = _mm512_unpackhi_epi32(s[4], s[5]);
    tmp0[6] = _mm512_unpacklo_epi32(s[6], s[7]);
    tmp0[7] = _mm512_unpackhi_epi32(s[6], s[7]);
    tmp1[0] = _mm512_unpacklo_epi64(tmp0[0], tmp0[2]);
    tmp1[1] = _mm512_unpackhi_epi64(tmp0[0], tmp0[2]);
    tmp1[2] = _mm512_unpacklo_epi64(tmp0[1], tmp0[3]);
    tmp1[3] = _mm512_unpackhi_epi64(tmp0[1], tmp0[3]);
    tmp1[4] = _mm512_unpacklo_epi64(tmp0[4], tmp0[6]);
    tmp1[5] = _mm512_unpackhi_epi64(tmp0[4], tmp0[6]);
    tmp1[6] = _mm512_unpacklo_epi64(tmp0[5], tmp0[7]);
    tmp1[7] = _mm512_unpackhi_epi64(tmp0[5], tmp0[7]);
    
    __m512i idx1 = _mm512_set_epi64(0x9ll,0x8ll,0x1ll,0x0ll,0xdll,0xcll,0x5ll,0x4ll); // the value to load the first 256 bits of b || the first 256 bits of a 
    __m512i idx2 = _mm512_set_epi64(0xbll,0xall,0x3ll,0x2ll,0xfll,0xell,0x7ll,0x6ll); // the value to load the last 256 bits of b || the last 256 bits of a 
    
    
    s[0] = _mm512_permutex2var_epi64(tmp1[0], idx1, tmp1[4]); 
    s[1] = _mm512_permutex2var_epi64(tmp1[1], idx1, tmp1[5]);
    s[2] = _mm512_permutex2var_epi64(tmp1[2], idx1, tmp1[6]);
    s[3] = _mm512_permutex2var_epi64(tmp1[3], idx1, tmp1[7]);
    s[4] = _mm512_permutex2var_epi64(tmp1[0], idx2, tmp1[4]);
    s[5] = _mm512_permutex2var_epi64(tmp1[1], idx2, tmp1[5]);
    s[6] = _mm512_permutex2var_epi64(tmp1[2], idx2, tmp1[6]);
    s[7] = _mm512_permutex2var_epi64(tmp1[3], idx2, tmp1[7]);    
}
                     ...
                     unsigned long long len) 
{
    ...
    while(i < len) {
        ...
        memcpy(&ctx->msgblocks[64*8], d8 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*9], d9 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*10], d10 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*11], d11 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*12], d12 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*13], d13 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*14], d14 + i, bytes_to_copy);
        memcpy(&ctx->msgblocks[64*15], d15 + i, bytes_to_copy);
        ctx->datalen += bytes_to_copy;
        i += bytes_to_copy;
        if (ctx->datalen == 64) {
            sha256_transform16x(ctx, ctx->msgblocks);
            ctx->msglen += 512; 
            ctx->datalen = 0;
        }        
    }
                     ...
                     unsigned char *out15) 
{
    ...
    __m512i h;
    h = BYTESWAP(ctx->s[0]);
    STORE(out0, _mm512_castsi512_si256(h));
    STORE(out8, _mm512_extracti64x4_epi64(h,1));

    h = BYTESWAP(ctx->s[1]);
    STORE(out1, _mm512_castsi512_si256(h));
    STORE(out9, _mm512_extracti64x4_epi64(h,1));
    
    h = BYTESWAP(ctx->s[2]);
    STORE(out2, _mm512_castsi512_si256(h));
    STORE(out10, _mm512_extracti64x4_epi64(h,1));

    h = BYTESWAP(ctx->s[3]);
    STORE(out3, _mm512_castsi512_si256(h));
    STORE(out11, _mm512_extracti64x4_epi64(h,1));

    h = BYTESWAP(ctx->s[4]);
    STORE(out4, _mm512_castsi512_si256(h));
    STORE(out12, _mm512_extracti64x4_epi64(h,1));

    h = BYTESWAP(ctx->s[5]);
    STORE(out5, _mm512_castsi512_si256(h));
    STORE(out13, _mm512_extracti64x4_epi64(h,1));

    h = BYTESWAP(ctx->s[6]);
    STORE(out6, _mm512_castsi512_si256(h));
    STORE(out14, _mm512_extracti64x4_epi64(h,1));

    h = BYTESWAP(ctx->s[7]);
    STORE(out7, _mm512_castsi512_si256(h));
    STORE(out15, _mm512_extracti64x4_epi64(h,1));
...
void sha256_transform16x(sha256ctx *ctx, const unsigned char *data) {
    ...
    int i;
    __m512i l1,l2;
    __m512i idx = _mm512_set_epi64(0xbll,0xall,0x9ll,0x8ll,0x3ll,0x2ll,0x1ll,0x0ll);
    // Load words and transform data correctly
    for(i = 0; i < 8; i++) {
        l1 = LOAD(data + 64*i);
        l2 = LOAD(data + 64*(i+8));
        w[i] = BYTESWAP(_mm512_permutex2var_epi64(l1, idx, l2));

        l1 = LOAD(data + 32 + 64*i);
        l2 = LOAD(data + 32 + 64*(i+8));
        w[i + 8] = BYTESWAP(_mm512_permutex2var_epi64(l1, idx, l2));
    }
