 ...
 *
 * This expects tree_addrx16 to be initialized to 16 parallel addr structures for
 ...
 * most real node will by the actual root (and the other 15 nodes will be
 * garbage).  We follow the same thashx16 logic so that the 'extract
 * authentication path components' part of the loop is still executed (and
 * to simplify the code somewhat)
 *
 * This currently assumes tree_height >= 4; I suspect that doing an adjusting
 * idx, addr_idx on the gen_leafx16 call if tree_height < 4 would fix it;
 */
void treehashx16(unsigned char *root, unsigned char *auth_path,
                ...
                uint32_t tree_addrx16[16*8],
                void *info)
{
    
    /* This is where we keep the intermediate nodes */
    unsigned char stackx16[tree_height*16*SPX_N];
    ...
    uint32_t idx;
    uint32_t max_idx = (1 << (tree_height-4)) - 1;
    for (idx = 0;; idx++) {
        unsigned char current[16*SPX_N];   /* Current logical node */
        gen_leafx16( current, sk_seed, pub_seed, 16*idx + idx_offset,
                    info );
        ...
        for (h=0;; h++, internal_idx >>= 1, internal_leaf >>= 1) {
            ...
            if (h >= tree_height - 4) {
                if (h == tree_height) {
                    ...
                    memcpy( root, &current[15*SPX_N], SPX_N );
                    return;
                }
             ...
             */
            if ((((internal_idx << 4) ^ internal_leaf) & ~0xf) == 0) {
                memcpy( &auth_path[ h * SPX_N ],
                        &current[(((internal_leaf&15)^1) + prev_left_adj) * SPX_N],
                        SPX_N );
            }
            ...
            for (j = 0; j < 16; j++) {
                set_tree_height(tree_addrx16 + j*8, h + 1);
                set_tree_index(tree_addrx16 + j*8,
                     (16/2) * (internal_idx&~1) + j - left_adj + internal_idx_offset );
            }
            unsigned char *left = &stackx16[h * 16 * SPX_N];
            thashx16(&current[0 * SPX_N],
                     &current[1 * SPX_N],
                     &current[2 * SPX_N],
                     &current[3 * SPX_N],
                     &current[4 * SPX_N],
                     ...
                     &current[12 * SPX_N],
                     &current[14 * SPX_N],
                     2, pub_seed, tree_addrx16);
        }

        /* We've hit a left child; save the current for when we get the */
        /* corresponding right right */
        memcpy( &stackx16[h * 16 * SPX_N], current, 16 * SPX_N);
    }
}
