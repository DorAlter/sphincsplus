...
#include "hash.h"
#include "16_times/hashx16.h"
#include "thash.h"
#include "16_times/thashx16.h"
#include "address.h"
#include "8_times/utilsx8.h"
#include "8_times/hashx8.h"
#include "8_times/thashx8.h"
...
static void fors_gen_skx16(unsigned char *sk0,
                           unsigned char *sk1,
                           unsigned char *sk2,
                           unsigned char *sk3,
                           unsigned char *sk4,
                           ...
                           unsigned char *sk13,
                           unsigned char *sk14,
                           unsigned char *sk15, const unsigned char *sk_seed,
                           uint32_t fors_leaf_addrx16[16*8])
{
    prf_addrx16(sk0, sk1, sk2, sk3, sk4, sk5, sk6, sk7,
                sk8, sk9, sk10, sk11, sk12, sk13, sk14, sk15,
                sk_seed, fors_leaf_addrx16);
}
...
static void fors_sk_to_leafx16(unsigned char *leaf0,
                               unsigned char *leaf1,
                               unsigned char *leaf2,
                               unsigned char *leaf3,
                               unsigned char *leaf4,
                               ...
                               unsigned char *leaf8,
                               unsigned char *leaf9,
                               unsigned char *leaf10,
                               unsigned char *leaf11,
                               unsigned char *leaf12,
                               unsigned char *leaf13,
                               unsigned char *leaf14,
                               unsigned char *leaf15,
                               ...
                               const unsigned char *sk8,
                               const unsigned char *sk9,
                               const unsigned char *sk10,
                               const unsigned char *sk11,
                               const unsigned char *sk12,
                               const unsigned char *sk13,
                               const unsigned char *sk14,
                               const unsigned char *sk15,
                               const unsigned char *pub_seed,
                               uint32_t fors_leaf_addrx16[16*8])
{
    thashx16(leaf0, leaf1, leaf2, leaf3, leaf4, leaf5, leaf6, leaf7,
             leaf8, leaf9, leaf10, leaf11, leaf12, leaf13, leaf14, leaf15,
             sk0, sk1, sk2, sk3, sk4, sk5, sk6, sk7,
             sk8, sk9, sk10, sk11, sk12, sk13, sk14, sk15,
             1, pub_seed, fors_leaf_addrx16);
}

struct fors_gen_leaf_infox16 {
    uint32_t leaf_addrx[16*8];
};
...
static void fors_gen_leafx16(unsigned char *leaf,
                            const unsigned char *sk_seed,
                            const unsigned char *pub_seed,
                            uint32_t addr_idx, void *info)
{
    struct fors_gen_leaf_infox16 *fors_info = info;
    uint32_t *fors_leaf_addrx16 = fors_info->leaf_addrx;
    unsigned int j;

    /* Only set the parts that the caller doesn't set */
    for (j = 0; j < 16; j++) {
        set_tree_index(fors_leaf_addrx16 + j*8, addr_idx + j);
    }

    fors_gen_skx16(leaf + 0*SPX_N,
                   leaf + 1*SPX_N,
                   leaf + 2*SPX_N,
                   leaf + 3*SPX_N,
                   leaf + 4*SPX_N,
                   ...
                   leaf + 15*SPX_N,
                   sk_seed, fors_leaf_addrx16);
    fors_sk_to_leafx16(leaf + 0*SPX_N,
                       leaf + 1*SPX_N,
                       leaf + 2*SPX_N,
                       leaf + 3*SPX_N,
                       leaf + 4*SPX_N,
...
static void fors_gen_leafx8(unsigned char *leaf,
                            ...
                            uint32_t addr_idx, void *info)
{
    struct fors_gen_leaf_infox8 *fors_info = info;
...
{
    if(SPX_TREE_HEIGHT >= 4) // checks if we can do 16 times 
    {
        uint32_t indices[SPX_FORS_TREES];
        unsigned char roots[SPX_FORS_TREES * SPX_N];
        uint32_t fors_tree_addr[16*8] = {0};
        struct fors_gen_leaf_infox16 fors_info = {0};
        ...
        for (i = 0; i < SPX_FORS_TREES; i++) {
            ...
            treehashx16(roots + i*SPX_N, sig, sk_seed, pub_seed,
                        indices[i], idx_offset, SPX_FORS_HEIGHT, fors_gen_leafx16,
        ...
        struct fors_gen_leaf_infox8 fors_info = {0};
        uint32_t *fors_leaf_addr = fors_info.leaf_addrx;
        uint32_t fors_pk_addr[8] = {0};
        uint32_t idx_offset;
        unsigned int i;
